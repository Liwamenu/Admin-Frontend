...
import { updateRole, resetUpdateRole } from "../../redux/roles/updateRoleSlice";
import { getPermissions } from "../../redux/roles/getPermissionsSlice";

const EditRolePopup = ({ roleData: initialRoleData }) => {
  const { setPopupContent } = usePopup();
  const dispatch = useDispatch();
  const toastId = useRef();

  const { success, loading, error } = useSelector((state) => state.updateRole);
  const { permissions } = useSelector((state) => state.getPermissions);

  const [roleData, setRoleData] = useState({
    id: initialRoleData?.id || "",
    name: initialRoleData?.role || "",
    description: initialRoleData?.description || "",
    isActive: initialRoleData?.isActive || true,
  });
  ...
  useEffect(() => {
    if (success) {
      ...
      toast.success("Rol başarıyla güncellendi", { id: "role-success" });
    ...
    <main className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
      <div className="w-full pt-8 pb-4 bg-[--white-1] rounded-lg border-2 border-solid border-[--border-1] text-[--black-2] max-w-3xl max-h-[95vh] overflow-y-auto">
        <div className="flex flex-col bg-[--white-1] relative px-6">
          ...
          <h1 className="text-2xl font-bold mb-6">Rolü Düzenle</h1>
                  ...
                  {permissions.data.map((module) => (
                    <div key={module.module}>
                      <div className="flex items-center justify-between mb-2 gap-2">
                        ...
                        <CustomCheckbox
                          id={`edit-select-all-${module.module}`}
                          label="Tumu"
